...
router.get('/create/:listingId', isLoggedIn, wrapAsync(async (req, res) => {
...
router.post('/create/:listingId', isLoggedIn, validateBooking, wrapAsync(async (req, res) => {
    const { listingId } = req.params; // Get ID directly from URL
...
router.put("/:id/edit", isLoggedIn, validateBooking, wrapAsync(async (req, res) => {
    const { id } = req.params;
    const { checkin, checkout, adults, children, price } = req.body.booking;  
